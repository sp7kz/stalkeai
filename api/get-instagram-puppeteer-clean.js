...
module.exports = async (req, res) => {
  ...
  try {
      ...
      for (const cookie of cookies) {
        ...
        if (name && value) {
          try {
            ...
            console.log(`[Puppeteer API] Erro ao setar cookie ${name}:`, e.message);
          }
        }
      }
      console.log('[Puppeteer API] Cookies aplicados:', cookies.length);
    ...
    const profileData = await page.evaluate(async (username) => {
      ...
      // Tenta buscar lista de seguindo (following)
      if (result.user_id) {
        console.log("[Puppeteer API] Fetching following list for user_id:", result.user_id);
        try {
          const followingResp = await fetch(
            `https://www.instagram.com/api/v1/friendships/${result.user_id}/following/?count=10`,
            {
              credentials: 'include',
              headers: {
                'x-ig-app-id': '936619743392459',
          ...
          if (followingResp.ok) {
            ...
            result.following_list = users.slice(0, 10).map(u => ({
              username: u.username || '',
            ...
            console.log("[Puppeteer API] Following list loaded:", result.following_list.length, "users");
          } else {
            console.log("[Puppeteer API] Following endpoint returned status:", followingResp.status);
          }
        } catch (err) {
          console.log("[Puppeteer API] Error fetching following list:", err.message);
        }
      }
